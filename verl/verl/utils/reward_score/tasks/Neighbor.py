...
class Neighbor_Task(NPTask):
    ...
    def check_solution(self, problem_id, response, graph=None, problem_text=None):
        ...
        if matches:
            ...
            # 从problem_text中获取node1和node2
            # 测试代码
            # test_text = "Please identify the common neighbors of node1 and node2 in this network"
            ...
            # #print(f"Node1 id: {node1_id}")
            # #print(f"Node2 id: {node2_id}")
            # if self.is_feasible(g, node_list, problem_id, node1, node2):
            if self.is_feasible(g, node_list, problem_id, node1_id, node2_id):
                #print(f"Valid Solution: {len(set(node_list))}")
    ...
    def is_feasible(self, g, node_list, problem_id, node1, node2):
        # 要改成从problem_text中确定node1和node2
