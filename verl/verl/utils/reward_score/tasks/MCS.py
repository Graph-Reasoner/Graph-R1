...
class MCS_Task(NPTask):
    ...
    def check_solution(self, problem_id, response, graph=None, problem_text=None):
        """Check the solution for MCS problem."""
        #print("\n---MCS CHECK SOLUTION DEBUG---")
            ...
            try:
                ...
                if molecule_a_indices and molecule_b_indices and len(molecule_a_indices) == len(molecule_b_indices):
                    ...
                    if graph:
                        ...
                        g1, g2 = graph # 不确定能不能直接这么解包，试试先
                        # # #print(f"g1: {g1}")
                        # # #print(f"g2: {g2}")
                        subgraph1 = g1.subgraph(molecule_a_indices)
                        subgraph2 = g2.subgraph(molecule_b_indices)
                        # # #print(f"subgraph1: {subgraph1}")
                        # # #print(f"subgraph2: {subgraph2}")
                        if nx.is_isomorphic(subgraph1, subgraph2):
                            #print(f"Valid Solution: {len(molecule_a_indices)}")
